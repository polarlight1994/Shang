...
parser.add_argument("--sql", type=str, help="The script to build the sql database", required=True)
parser.add_argument("--sdc", type=str, help="The path to which the sdc script will be written", required=True)
args = ParseOptions("--period", type=str, help="The clock period", required=True)
...
def generate_constraint(**kwargs) :
  if kwargs['thu'] == "shang-critical-path-node" :
