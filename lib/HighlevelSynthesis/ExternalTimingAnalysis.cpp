...
EnableTimingConstraint("vast-external-enable-timing-constraint",
  cl::desc("Use timing constraints to tweak the data pathin external tool"),
  cl::init(false));
...
void GenerateTimingConstraint(raw_ostream &O, VASTSelector *Dst, VASTSeqValue *Src,
                              VASTValue *Thu, float delay) {
  ...
  delay = std::max(delay - 0.5f, 0.0f);
  unsigned Cycles = ceil(delay);
  if (Cycles <= 1)
    return;
...
void ExternalTimingAnalysis::extractTimingForSelector(raw_ostream &TclO,
  ...
  for (leaf_iterator I = AllLeaves.begin(), E = AllLeaves.end(); I != E; ++I) {
    ...
    GenerateTimingConstraint(TimingSDCO, Sel, Src, 0, CriticalDelay.lookup(Src));
  }
  ...
  for (ann_iterator I = Sel->ann_begin(), E = Sel->ann_end(); I != E; ++I) {
    ...
    for (unsigned i = 0; i < Slots.size(); ++i) {
      ...
      for (leaf_iterator LI = CurLeaves.begin(), LE = CurLeaves.end();
        ...
        GenerateTimingConstraint(TimingSDCO, Sel, Leaf, Expr, delay);
      }
...
bool ExternalTimingAnalysis::analysisWithSynthesisTool() {
  ...
  writeTimingAnalysisDriver(PrjTclO, TimingExtractTcl, EnablePAR);
  if (EnablePAR)
