...
struct PathRef {
  ...
  VASTValue *From;
  // The corresponding root of the combinational cone.
  VASTValue *Root;

  PathRef(unsigned Idx) : Idx(Idx), From(0), Root(0) {}
};
...
struct ExternalTimingAnalysis : public VASTModulePass {
  ...
  unsigned allocatePathRef(VASTValue *From, VASTValue *Root) {
    unsigned Idx = PathRefs.size();
    PathRef Ref(Idx);
    Ref.From = From;
    Ref.Root = Root;
    PathRefs.push_back(Ref);
    ++NumQueriesWritten;
    return Idx;
  }
...
void ExternalTimingAnalysis::extractTimingForSelector(raw_ostream &O,
  ...
  for (iterator I = CI.begin(), E = CI.end(); I != E; ++I) {
    VASTValue *Thu = I->first;
    DEBUG(dbgs() << GetObjectName(Thu) << '\n');
    bool ThuIsReg = isa<VASTSeqValue>(Thu);
    ...
    for (src_iterator SI = Srcs.begin(), SE = Srcs.end(); SI != SE; ++SI) {
      DEBUG(dbgs().indent(2) << "Root: " << GetObjectName(SI->first)
                             ...
                             << '\n');
      VASTValue *Root = SI->first;
      VASTSeqValue *Src = SI->second;
      bool RootIsReg = isa<VASTSeqValue>(Root);
      if (ThuIsReg || RootIsReg) {
        // if (ThuIsReg && RootIsReg)
        // TODO: Annotate the trivial path in the TNL.
        continue;
      }

      // Now both Thu and Root is not register, and hence Thu != Src.
      // extractTimingForPath(O, Sel, Thu, Src);
      if (Thu != Root) {
        unsigned Idx = allocatePathRef(Src, Root);
        extractTimingForPath(O, Root, Thu, Src, Idx);
      }

      unsigned Idx = allocatePathRef(Src, Root);
      extractTimingForPath(O, Thu, (VASTValue*)0, Src, Idx);
    }
  }
}
