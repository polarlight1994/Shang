...
struct LowerPseudoPHIs : public VASTModulePass {
  ...
  VASTModule *VM;
  std::map<VASTRegister*, SeqLiveVariables::VarInfo> CachedSrc;

  void resetCache() {
    CachedSrc.clear();
  }

  SeqLiveVariables::VarInfo &getCachedVI(VASTRegister *V) {
    return CachedSrc[V];
  }
  ...
  void lowerPseudoPHI(VASTRegister *V, VASTValPtr ForwordedValue);
  void extendLiveInterval(VASTRegister *SrcV, VASTRegister *DstV);
};
}
...
// pn = phi [ incoming, ...] [pn|undef, ...]+
static VASTValPtr isPseudoPHI(VASTRegister *V) {
  if (V->getValType() != VASTRegister::Data || V->size() <= 1)
    return VASTValPtr();
  
  VASTValPtr Forwarded;

  for (VASTRegister::const_iterator I = V->begin(), E = V->end(); I != E; ++I) {
    VASTValPtr Val = *I;
...
bool LowerPseudoPHIs::runOnVASTModule(VASTModule &VM) {
  ...
  std::vector<std::pair<VASTRegister*, VASTHandle> > Worklist;
...
void LowerPseudoPHIs::extendLiveInterval(VASTRegister *SrcV, VASTRegister *DstV)
{
...
void LowerPseudoPHIs::lowerPseudoPHI(VASTRegister *V, VASTValPtr ForwardedValue)
{
  // Extend the live interval of the forwarded value.
  std::set<VASTRegister*> SrcVals;
