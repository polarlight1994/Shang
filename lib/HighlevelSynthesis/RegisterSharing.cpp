...
public:
  ...
  SeqLiveInterval(VASTRegister *SV, SeqLiveVariables *LVS) : SV(SV) {
    if (SV == 0) return;
  ...
  VASTRegister *get() const { return SV; }
  VASTRegister *operator*() const { return get(); }
  VASTRegister *operator->() const { return get(); }
...
private:
  typedef DenseMap<VASTRegister*, NodeTy*> NodeMapTy;
...
public:
  ...
  NodeTy *operator[](VASTRegister *N) const { return Nodes.lookup(N); }

  NodeTy *GetOrCreateNode(VASTRegister *V, SeqLiveVariables *LVS) {
...
struct RegisterSharing : public VASTModulePass {
  ...
  void mergeSeqVal(VASTRegister *ToV, VASTRegister *FromV);
};
}
...
bool RegisterSharing::runOnVASTModule(VASTModule &VM) {
  ...
  for (iterator I = VM.seqval_begin(), IE = VM.seqval_end(); I != IE; ++I) {
    if (I->getValType() != VASTRegister::Data) continue;
...
void RegisterSharing::mergeLI(SeqLiveInterval *From, SeqLiveInterval *To) {
  ...
  VASTRegister *FromV = From->get(), *ToV = To->get();
...
void RegisterSharing::mergeSeqVal(VASTRegister *ToV, VASTRegister *FromV) {
