...
bool isWantedMask(SIRBitMask Mask) {
  APInt KnownZeros = Mask.getKnownZeros();
  APInt KnownOnes = Mask.getKnownOnes();

  std::vector<char> BitGroup;
  unsigned BitWidth = Mask.getMaskWidth();
  for (unsigned i = 0; i < BitWidth; ++i) {
    if (KnownZeros[BitWidth - 1 - i] == 1)
      BitGroup.push_back('0');
    else if (KnownOnes[BitWidth - 1 - i] == 1)
      BitGroup.push_back('1');
    else
      BitGroup.push_back('x');
  }

  unsigned LeadingZero = 0;
  for (unsigned i = 0; i < BitWidth; ++i) {
    if (BitGroup[i] == '0')
      LeadingZero += 1;
    else
  ...
  unsigned MiddleX = 0;
  for (unsigned i = LeadingZero; i < BitWidth; ++i) {
    if (BitGroup[i] == 'x')
      MiddleX += 1;
    else
      break;
  ...
  unsigned TailingZero = 0;
  for (unsigned i = LeadingZero + MiddleX; i < BitWidth; ++i) {
    if (BitGroup[i] == '0')
      TailingZero += 1;
    else
      break;
  ...
  if (LeadingZero + MiddleX + TailingZero != BitWidth)
    return true;
  else
    return false;
}
...
Value *SIRDatapathBLO::optimizeMulInst(Instruction *MulInst) {
  ...
  if (SM->hasBitMask(LHS) || SM->hasBitMask(RHS)) {
    SIRBitMask LHSMask = SM->hasBitMask(LHS) ? SM->getBitMask(LHS) : SIRBitMask(Builder.getBitWidth(LHS));
    SIRBitMask RHSMask = SM->hasBitMask(RHS) ? SM->getBitMask(RHS) : SIRBitMask(Builder.getBitWidth(RHS));

    if (LHSMask.hasAnyBitKnown() || RHSMask.hasAnyBitKnown()) {
       if (!LHSMask.isAllBitKnown() && !RHSMask.isAllBitKnown()) {
         if (isWantedMask(LHSMask) || isWantedMask(RHSMask)) {
           if (VisitedMulInst.count(MulInst))
             return MulInst;

           MulMaskOutput << MulInst->getName() << "\n";
           errs() << MulInst->getName() << "\n";
...
//     Value *LHSKnownZeros = Builder.createIntegerValue(LHSMask.getKnownZeros());
//     Value *LHSKnownOnes = Builder.createIntegerValue(LHSMask.getKnownOnes());
//     Value *MaskedLHS = Builder.createSOrInst(Builder.createSAndInst(LHS, Builder.createSNotInst(LHSKnownZeros, LHSKnownZeros->getType(), MulInst, true), LHS->getType(), MulInst, true), LHSKnownOnes, LHS->getType(), MulInst, true);
// 
//     Value *RHSKnownZeros = Builder.createIntegerValue(RHSMask.getKnownZeros());
//     Value *RHSKnownOnes = Builder.createIntegerValue(RHSMask.getKnownOnes());
//     Value *MaskedRHS = Builder.createSOrInst(Builder.createSAndInst(RHS, Builder.createSNotInst(RHSKnownZeros, RHSKnownZeros->getType(), MulInst, true), RHS->getType(), MulInst, true), RHSKnownOnes, RHS->getType(), MulInst, true);
//     
//     MulInst->setOperand(0, MaskedLHS);
//     MulInst->setOperand(1, MaskedRHS);
  }
...
Value *SIRDatapathBLO::optimizeRegAssignInst(Instruction *RegAssignInst) {
  ...
  Mask.evaluateMask(RegAssignInst, SM, TD);
...
void SIRDatapathBLO::computeMask(Instruction *Inst) {
  unsigned BitWidth = Builder.getBitWidth(Inst);
  ...
  if (IntrinsicInst *II = dyn_cast<IntrinsicInst>(Inst))
    if (II->getIntrinsicID() == Intrinsic::shang_mul)
      optimizeMulInst(Inst);

  SM->IndexVal2BitMask(Inst, Mask);
}

bool SIRDatapathBLO::optimizeValue(Value *Val) {
  if (SM->hasBitMask(Val))
    return false;
...
bool SIRDatapathBLO::optimizeRegister(SIRRegister *Reg) {
  ...
  Changed |= optimizeValue(RegAssignInst);
...
struct SIRBitLevelOpt : public SIRPass {
  ...
  unsigned isUserElement(IntrinsicInst *Root, raw_fd_ostream &ChainOutput);
  void printMask(SIR &SM, DataLayout &TD, raw_fd_ostream &Output);
...
std::set<IntrinsicInst *> VisitedChainInst;

unsigned SIRBitLevelOpt::isUserElement(IntrinsicInst *Root, raw_fd_ostream &ChainOutput) {
  typedef Value::use_iterator use_iterator;
  std::vector<std::pair<IntrinsicInst *, use_iterator> > VisitStack;

  VisitStack.push_back(std::make_pair(Root, Root->use_begin()));

  unsigned Depth = 0;

  while(!VisitStack.empty()) {
    IntrinsicInst *CurNode = VisitStack.back().first;
    use_iterator &I = VisitStack.back().second;

    assert((CurNode == Root) || (CurNode->getIntrinsicID() == Intrinsic::shang_add || CurNode->getIntrinsicID() == Intrinsic::shang_addc) && "Unexpected type!");

    // All children of current node have been visited.
    if (I == CurNode->use_end()) {
      VisitStack.pop_back();

      if (Depth != 0) {
        CurNode->print(ChainOutput);

      for (unsigned i = 0; i < CurNode->getNumOperands() - 1; ++i) {
        Value *Operand = CurNode->getOperand(i);

        bool hasOpMult = false;
        if (IntrinsicInst *OperandInst = dyn_cast<IntrinsicInst>(Operand)) {
          if (OperandInst->getIntrinsicID() == Intrinsic::shang_mul) {
            bool isContantMult = false;

            for (unsigned j = 0; j < OperandInst->getNumOperands() - 1; ++j) {
              Value *OperandInstOp = OperandInst->getOperand(j);

              if (isa<ConstantInt>(OperandInstOp)) {
                isContantMult = true;
                break;
              }
            }
    ...
    Value *UserVal = *I;
    ++I;
    IntrinsicInst *UserInst = dyn_cast<IntrinsicInst>(UserVal);
    ...
    if (VisitedChainInst.count(UserInst))
      continue;

    if (UserInst->getIntrinsicID() == Intrinsic::shang_add || UserInst->getIntrinsicID() == Intrinsic::shang_addc) {
      VisitStack.push_back(std::make_pair(UserInst, UserInst->use_begin()));
      VisitedChainInst.insert(UserInst);
      Depth++;
    }
  }
...
void SIRBitLevelOpt::printMask(SIR &SM, DataLayout &TD, raw_fd_ostream &Output) {
