...
struct OnTheFlySelectorSynthesis : public MinimalExprBuilderContext {
  CachedStrashTable &CST;
  VASTExprBuilder Builder;

  OnTheFlySelectorSynthesis(DatapathContainer &Datapath, CachedStrashTable &CST)
    : MinimalExprBuilderContext(Datapath), CST(CST), Builder(*this) {}

  void synthesizeSelector(VASTSelector *Sel);

  static void run(VASTModule &VM, CachedStrashTable &CST) {
    OnTheFlySelectorSynthesis SS(VM, CST);
...
void OnTheFlySelectorSynthesis::synthesizeSelector(VASTSelector *Sel) {
...
bool DataflowAnnotation::externalDelayAnnotation(VASTModule &VM) {
  OnTheFlySelectorSynthesis::run(VM, getAnalysis<CachedStrashTable>());
